...
def load_matrices_results(counter, errors):
    ...
    for matrix in tqdm(db.matrices_results.aggregate([
...
def distplot_instances(df, x_vals, y_vals, impossible=None):
    ...
    fig.savefig("distplot_%s_%s%s.png"
                % (x_vals, y_vals, IMPOSSIBLE_SUFFIX[impossible]))
    return fig
...
def histplot_instances(df, impossible=None):
    ...
    ax.set_yscale('log')
    fig.savefig(f"histplot_tp_length%s.png" % IMPOSSIBLE_SUFFIX[impossible])
    return fig
...
fig.savefig(f"histplot_tp_length_combined.png")
show_fig(fig)
...
def relplot(df, x_axis, y_axis, impossible, sample_count=None, sample_frac=None):
    ...
    if impossible is not None:
        df = df[df["exp_possible"] == (not impossible)]
    x_buckets = pd.cut(df[x_axis], BUCKETS[x_axis])
    ...
    ax.set_xlim(max(BUCKETS[x_axis]) / (-100), max(BUCKETS[x_axis]))
    ...
    fig.savefig(
        "relplot_%s_%s%s.png" % (y_axis, x_axis, IMPOSSIBLE_SUFFIX[impossible]))
    return fig
...
def plot_heatmaps_buckets(df, value, impossible=None):
    ...
    fig.savefig("heatmap_%s%s_buckets.png" % (value, IMPOSSIBLE_SUFFIX[impossible]))
    return fig
