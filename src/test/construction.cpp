...
go_bandit([]() {
    describe("PCTree construction", []() {
        it("allows creating a trivial instance", []() {
            ...
            AssertThat(std::equal(tree.getLeaves().begin(), tree.getLeaves().end(), leaves.begin(), leaves.end()), IsTrue());
        ...
        it("correctly handles the first case where JPPZanetti fails", []() {
            ...
            AssertThat(tree.checkValid(), IsTrue());
            AssertThat(tree.uniqueID(uid_utils::nodeToPosition), Equals("8:(7:[5, 4], 6, 3, 2, 1, 0)"));
            tree.makeConsecutive({leaves.at(3), leaves.at(4), leaves.at(5)});
            AssertThat(tree.checkValid(), IsTrue());
            AssertThat(tree.uniqueID(uid_utils::nodeToPosition), Equals("9:(8:[7:[5, 4], 3], 6, 2, 1, 0)"));
            tree.makeConsecutive({leaves.at(0), leaves.at(1)});
            AssertThat(tree.checkValid(), IsTrue());
            AssertThat(tree.uniqueID(uid_utils::nodeToPosition), Equals("10:(9:[8:[5, 4], 3], 7:[1, 0], 6, 2)"));
            tree.makeConsecutive({leaves.at(1), leaves.at(2)});
            AssertThat(tree.checkValid(), IsTrue());
            AssertThat(tree.uniqueID(uid_utils::nodeToPosition), Equals("10:[9:[8:[5, 4], 3], 7:[2, 1, 0], 6]"));
            tree.makeConsecutive({leaves.at(2), leaves.at(3), leaves.at(4), leaves.at(5)});
            AssertThat(tree.checkValid(), IsTrue());
            AssertThat(tree.uniqueID(uid_utils::nodeToPosition), Equals("9:[6, 8:[7:[5, 4], 3], 2, 1, 0]"));
            tree.makeConsecutive({leaves.at(3), leaves.at(4), leaves.at(5), leaves.at(6)});
            AssertThat(tree.checkValid(), IsTrue());
            AssertThat(tree.uniqueID(uid_utils::nodeToPosition), Equals("9:[6, 8:[7:[5, 4], 3], 2, 1, 0]"));
            tree.makeConsecutive({leaves.at(3), leaves.at(4)});
            AssertThat(tree.checkValid(), IsTrue());
            AssertThat(tree.uniqueID(uid_utils::nodeToPosition), Equals("8:[6, 7:[5, 4, 3], 2, 1, 0]"));
            tree.makeConsecutive({leaves.at(2), leaves.at(3), leaves.at(4), leaves.at(5)});
            AssertThat(tree.checkValid(), IsTrue());
            AssertThat(tree.uniqueID(uid_utils::nodeToPosition), Equals("8:[6, 7:[5, 4, 3], 2, 1, 0]"));
        });
        ...
        it("is iterated correctly", []() {
            ...
            std::sort(nodes.begin(), nodes.end(), [](PCNode *a, PCNode *b) {
