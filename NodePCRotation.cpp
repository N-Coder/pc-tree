...
NodePCRotation::NodePCRotation(Graph &G, node end, const bool mapBundleEdges) :
    ...
    for (node n : order) {
        ...
        if (n == order[0]) {
            ...
            mergedLeaf = newNode(PCTree::PCNodeType::PNode, nullptr);
        } else {
        ...
        if (outEdges.size() > 1) {
            ...
            if (mergedLeaf->nodeType == PCTree::PCNodeType::PNode) {
...
node NodePCRotation::getTrivialPartnerPole() const {
    ...
    if (!isTrivial())
        return nullptr;
    node partner = graphNodeForInnerNode[getParent(getLeaves().front())];
    if (partner->degree() < 3)
        return nullptr;
    else
        return partner;
