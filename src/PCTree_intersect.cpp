...
bool pc_tree::PCTree::findNodeRestrictions(PCTree &applyTo, PCTreeNodeArray<PCNode *> &mapping,
    ...
    timestamp++;
    std::vector<PCNode *> labelLeaves(leaves.begin(), std::prev(leaves.end()));
    assignLabels(labelLeaves, &fullNodeOrder);
    ...
    for (PCNode *node: fullNodeOrder) {
        auto nonLeafNeighborIt = std::find_if(node->neighbors().begin(), node->neighbors().end(), [](PCNode *n) {
            return !n->isLeaf();
        });

        PCNode *startWith = nonLeafNeighborIt == node->neighbors().end() ? node->neighbors().first : *nonLeafNeighborIt;
        ...
        if (node != fullNodeOrder.back()) {
            ...
            applyTo.assignLabels(consecutiveOriginal, &nodeOrder);
            ...
            auto fbEnd1It = std::find_if(fullNeighbors.begin(), fullNeighbors.end(), [&](PCNode *n) {
                ...
                if (sib1->tempInfo().label != NodeLabel::Full) {
                    ebEnd1 = sib1;
                    return true;
                }
                PCNode *sib2 = partialNode->getNextNeighbor(sib1, n);
                if (sib2->tempInfo().label != NodeLabel::Full) {
                    ebEnd1 = sib2;
                    return true;
                }
                return false;
            ...
            int count = applyTo.findEndOfFullBlock(partialNode, ebEnd1, fbEnd1, fbEnd2, ebEnd2);
            ...
            while (current != ebEnd2) {
                OGDF_ASSERT(current->tempInfo().label == NodeLabel::Full);
...
void pc_tree::PCTree::restoreSubtrees(PCTreeNodeArray<std::vector<PCNode *>> &blockNodes,
    ...
    while (!queue.empty()) {
        ...
        for (int i = 0; i < degree; i++) {
            OGDF_ASSERT(!current->isLeaf() || leaves.at(current->nodeListIndex) == current);
            ...
            if (!blockNodes[current].empty()) {
                ...
                for (PCNode *n: subtreeNodes[current]) {
                    if (n->getNodeType() == PCNodeType::CNode) {
                        cNodeCount++;
                        OGDF_ASSERT(cNodes.at(n->nodeListIndex) == nullptr);
                        cNodes[n->nodeListIndex] = n;
                    } else {
                        registerNode(n);
                    }
                }
