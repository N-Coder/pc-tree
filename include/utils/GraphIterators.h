...
namespace pc_tree::hsu {
    ...
    class FilteringBFS : public std::iterator<std::input_iterator_tag, node> {
        ...
        void next() {
            ...
            if (m_descend(n)) {
                for (adjEntry adj : n->adjEntries) {
                    ...
                    if (!m_visited[twin] && m_visit(adj)) m_pending.append(twin);
                }
            }
            while (!m_pending.empty() && m_visited[m_pending.top()]) m_pending.pop();
        }
